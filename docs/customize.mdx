    ...
    <table className="w-full leading-normal">
        <thead>
            <tr className="border border-gray-300">
                <td className="p-4 font-bold">Variable</td>
                <td className="p-4 font-bold">Description</td>
            </tr>
        </thead>
        <tbody>
            {Array.from(new Set(props.utilities.map(u => u.themeKey))).filter(k => !!k).map(key => (
            <tr key={key} className="border border-gray-300 text-xs">
                ...
                <td className="p-4 text-gray-700">
                    ...
                    <span>
                        {props.utilities.filter(u => u.themeKey === key).map(u => (
                            <Fragment>
                                <span className="font-mono font-bold text-gray-900" key={u.key}>{u.key}</span>
